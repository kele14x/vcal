...
- Comments (LRM 3.2) separate those same tokens exactly as spaces do, so `8/*c*/'d5` is the same literal as `8'd5`, and `8 /*a*/ /*b*/ 'd 5` is allowed too. A `/` that is really division still lexes as division — `8 /*a*/ / 2` is `8 / 2`, not a malformed constant. An unterminated block comment anywhere in that lookahead is the usual `unterminated block comment` syntax error.
- `?` is a *z_digit*, so it belongs to the digit run of a *binary_value*, *octal_value*, or *hex_value*, which may interleave it with value digits: `8'h1?1` and `8'o7?7` are each a single literal. A *decimal_number* is narrower — after *decimal_base* it takes either an *unsigned_number* or one *x_digit* / *z_digit* / `?` followed by underscores — so `?` cannot follow any completed decimal value, whether a digit or an unknown one, and `8'dxx` / `8'dx_x` are syntax errors. Consequently `1'd1?2:3` is the conditional `1'd1 ? 2 : 3` and `8'dx?1:1` is `8'dx ? 1 : 1`, whereas `8'h1?2:3` lexes `8'h1?2` as one hex constant and then fails on the leftover `:3`.
