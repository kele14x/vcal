...
The LRM defines `statement ::= blocking_assignment ; | system_task_enable` inside a module, with declarations at the module-item level (Annex A.2). vcal has no module wrapper and no separate elaboration stage, so the REPL accepts a flat top-level stream `{ statement | system_task_enable | declaration | display_expression }`. `declaration` (`reg`, `integer`, `real`) is hoisted from module-item level so users can introduce a variable without a module. The calculator-mode production, which has no LRM counterpart, is non-empty:

```text
display_expression ::= expression { , [ expression ] }
                     | , [ expression ] { , [ expression ] }
```

A bare expression such as `a + 1` is the one-argument case, while `a, b` evaluates and displays both values. The argument-list shape otherwise follows `system_task_enable`'s `[ expression ] { , [ expression ] }` (Annex A.6.4), so a `display_expression` accepts null slots from leading, trailing, or adjacent commas exactly as a `$display` argument list does. At least one expression or comma token is required: a blank line is empty REPL input, not a zero-argument display expression, while explicit `$display()` remains a valid zero-argument task call. The comma exists only at this top level; it does not introduce a tuple value or a general comma operator, and assignment remains a statement (`a = 1; a` is valid sequencing, while `a = 1, a` is rejected).

Unformatted `display_expression` arguments retain their canonical vcal rendering and are space-separated. In a multi-argument `display_expression`, a string-style first argument activates the same format-control engine as `$display`, so `"a=%d", a` prints `a=10`; any arguments left unconsumed by format controls also render canonically, so `"label", 8'hff` prints `label 8'hff`. String style is expression metadata rather than a property inferred from packed bits: literals, grouping, and string-only concatenation / replication preserve it, while mixed or numeric operations do not, and a packed `reg` remains an ordinary numeric vector even when assigned a string literal. A lone string remains an ordinary canonical string echo (including quotes and escapes), preserving the established calculator behavior. Explicit `$display` remains distinct: it uses the LRM decimal default for unformatted integers (including arguments left unconsumed by format controls), is emitted as system-task output without an `Out[n]:` prefix, and is not suppressed by a trailing semicolon.
...
vcal implements Verilog string constants as packed 8-bit vectors. As a REPL convenience, bare string literals render as escaped quoted text even though their value is still the packed vector. String-only concatenation and replication preserve that display style (`{"A", "B"}` -> `"AB"`); mixed numeric concatenation and display-base casts render numerically (`{"A", 8'h42}` -> `16'h4142`, `$dec("AB")` -> `16'd16706`). Per LRM 5.2.3.3 the empty source literal `""` is equivalent to the ASCII NUL `"\0"` (value zero, distinct from the string `"0"`), so vcal treats it as one NUL byte and user syntax cannot create a zero-width vector. Zero-width vectors may still exist internally for debug or fault-tolerance paths, and numeric display renders them with one zero digit (`0'h0`, `0'd0`) — this rendering is a vcal display choice, not LRM-specified.

## Display tasks

vcal implements `$display` and `$write`, but deliberately supports only the format controls useful in the calculator-style REPL today: `%b`, `%o`, `%d`, `%h` / `%x`, `%s`, `%c`, real controls (`%e`, `%f`, `%g` and uppercase spellings), and `%%`. The `display_expression` format mode uses this same subset. For `%s` and `%c`, x/z bits in integer arguments are silently converted to zero bits before raw bytes are emitted; these controls do not fall back to canonical integer text. Null arguments from leading, trailing, or adjacent commas are accepted by all system tasks and by `display_expression`; for `$display` / `$write` and `display_expression` each null argument emits one space, while for `$finish` / `$stop` the null arguments are discarded along with all other arguments. The space is emitted only for a *supported* control — the control is validated first, so `$display("%q", )` reports the same unsupported-control error as `$display("%q", 5)` instead of silently printing a space. Format-control rejections are a static property of the input rather than a runtime condition: a format string can only be literal-derived, because string style survives no packed `reg` (iverilog agrees — `$display(s, x)` on `reg [63:0] s = "v=%d"` prints two numbers). They are therefore checked while the display argument list is prepared and surface as `Semantic error:`, alongside every other static-semantics rejection. Other LRM display controls are intentionally unsupported for now, including `%u` / `%z` unformatted data, `%t` simulation time, `%m` hierarchy path, strength formats, and field-width / precision modifiers. `$strobe`, `$monitor`, and file display tasks are also outside the current scope.
