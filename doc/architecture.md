...
`SelectKind` index / range / base / width sub-expressions aren't part of the annotated tree; `validate_select_kind_structure` runs each through its own `annotate` + `validate_annotated` pass (`validate_subexpr_structure`), mirroring how the evaluators route them through `evaluate_subexpr_as_integer`. The remaining raw-`Expr` helpers are `expression_is_real` (integer-vs-real dispatch in the public entry points, the select / lvalue meta walkers, and lib.rs's range-endpoint checks) and `is_indefinite_width` (the concatenation-operand check in `validate_annotated`).
