...
- Multi-statement input collapses the `In[n]/Out[n]` numbering. `1; 2; 3` prints three result lines under one `Out[0]:` and the prompt jumps to `In[1]`, breaking the one-input-one-result contract in [repl.md](repl.md). Either reject `;`-separated statements at top level, or advance the input counter per statement so each result gets its own `Out[n]:` label.

- Unsized `'sd` literals can silently flip negative when auto-widening lands the MSB on a set bit. `'sd9999999999999999999999999` is widened to 84 bits per the non-standard "use as many bits as the value needs" rule, and its bit 83 is set, so the value displays as `-84'sd9342813113834066795298817`. The user typed a positive literal and got a negative result with no diagnostic. Options: widen by one extra bit for signed unsized literals so the MSB is never set by the value alone, or emit a `Semantic warning:` when the chosen width forces the sign bit on.
