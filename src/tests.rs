...
#[test]
fn lhs_bit_select_assignment_is_rejected() {
    // LHS selects are out of scope; the statement dispatch only sees
    // `Identifier + Assign` as a blocking assignment, so `r[0] = 1'b1`
    // falls into the expression path and errors when the `=` shows up.
    let mut session = Session::new();
    session.eval("reg [7:0] r").expect("decl");
    let err = session
        .eval("r[0] = 1'b1")
        .expect_err("LHS select rejected");
    assert!(
        err.contains("unexpected token"),
        "error should mention unexpected token, got: {err}"
    );
